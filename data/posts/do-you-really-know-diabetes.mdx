...
* Bolus calculators (e.g. Roche’s [mySugr](https://mysugr.com/en) mobile app) - software that calculates the rapid-insulin units a patient should administer, based on the grams/portions of carbohydrates he is going to ingest and the current glucose value. The algorithm uses some previously configured variables like the insulin sensitivity factor and the insulin-to-carbs ratios (provided by the clinician) and can be personalized for different insulin types and hours of the day.
* Insulin pumps - small, digital devices that try to mimic a functional pancreas by continuously delivering small doses of insulin (basal rate) and variable amounts of insulin when eating (bolus). This sum depends on the quantification of the carbohydrate content of that meal. These devices allow for more advanced management of diabetes (to regulate the basal rate per hour or to regulate the bolus if the meal has a high-fat concentration.
...
![Artificial pancreas schema ](data/images/artificial-pancreas.jpg "Artificial pancreas")
